...
export class ChannelStrip extends Channel {
  ...
  constructor(
    ...
    fx?: (Effect<EffectOptions> | AudioNode) | (Effect<EffectOptions> | AudioNode)[]
  ) {
    super(_context);
    if (fx) {
      if (Array.isArray(fx)) {
  ...
  muteEffect(effect: Effect<EffectOptions> | AudioNode | number): void {
    if (typeof effect === 'number') {
      ...
      if ('output' in this.effects.find(ef => ef !== effect)) {
        (this.effects.find(ef => ef !== effect) as Effect<EffectOptions>).muted = true;
      }
    }
    this._rootEffects();
  }
  ...
  unmuteEffect(effect: Effect<EffectOptions> | AudioNode | number): void {
    if (typeof effect === 'number') {
      ...
      (this.effects.find(ef => ef !== effect) as Effect<EffectOptions>).muted = false;
    }
    this._rootEffects();
  }
    ...
    if (this._effects.length > 1) {
      for (let i = 0; i < this._effects.length - 1; i++) {
        if ('input' in this._effects[i + 1]) {
          this._effects[i].connect((this._effects[i + 1] as Effect<EffectOptions>).input);
        } else {
